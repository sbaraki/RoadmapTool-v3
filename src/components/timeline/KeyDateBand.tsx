...
function estimateLabelWidth(label: string) {
  return Math.max(190, label.length * 7.4 + 34)
}
...
export function KeyDateBand({ keyDates, timelineStart, timelineEnd, monthWidth, totalWidth }: KeyDateBandProps) {
  ...
  const laneHeight = 58
  const bandHeight = laneCount * laneHeight + 14
  ...
  return (
    ...
    >
      ...
      {instances.map(instance => {
        ...
        const top = instance.lane * laneHeight + 9
        return (
          <div key={instance.id} className="key-date-item" style={{ top }}>
            ...
            >
              <span className="key-date-label-title">{instance.title}</span>
              <span className="key-date-label-date">{instance.dateLabel}</span>
            </div>
            <div
              className="key-date-guide"
              style={{
                left,
                '--key-date-color': instance.color,
              } as CSSProperties}
