...
function resetExportSizing(container: HTMLElement) {
  ...
  container.style.removeProperty('--key-date-item-offset')
}
...
function applyPdfVerticalFill(container: HTMLElement, targetAspect: number) {
  ...
  container.style.setProperty('--key-date-item-offset', `${keyDateBandExtra / 2}px`)
}
